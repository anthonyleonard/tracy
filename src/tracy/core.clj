(ns ^{:doc "Fork of https://github.com/clojure/tools.trace/ (inline for now)"}
  tracy.core
  (:refer-clojure :exclude [pr-str])
  (:require [clojure.pprint]
            [puget.printer]))

;; --- START SLC monkey patch ---

(defn pr-str [& xs]
  (puget.printer/with-options {:width 100000000}
                              (apply str (map puget.printer/cprint-str xs))))

;; --- END SLC monkey patch ---
...
(defn ^{:skip-wiki true} trace-ns*
  ...
  No-op for clojure.core and uk.co.slc.tracy."
  [ns]
  (let [ns (the-ns ns)]
    (when-not ('#{clojure.core uk.co.slc.tracy} (.name ns))
...
(defn find-ns-starts-with [prefix]
  (filter #(-> %
               ns-name
               name
               (clojure.string/starts-with? prefix)) (all-ns)))

(defn trace-ns-starts-with [prefix]
  (let [ns-list (find-ns-starts-with prefix)]
    (println "Tracing ..." ns-list)
    (map #(trace-ns* %) ns-list)))

(defn untrace-ns-starts-with [prefix]
  (let [ns-list (find-ns-starts-with prefix)]
    (println "Untracing ..." ns-list)
    (map #(untrace-ns* %) ns-list)))
